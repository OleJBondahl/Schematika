...
- `src/schematika/catalog/__init__.py` — order `errors → identifiers → refs → parts → connectors → device → wires → routes → bom → nets → result → registry → cables` (breaks the `cables.py` → `registry.py` runtime-import cycle). Protected by the blanket `__init__.py` `I001` ignore in `pyproject.toml`, not a per-line `# noqa`.
