...
`Project` (Layer 4) exposes three additive chaining methods that delegate to owned builders: `route(*waypoints, net=...)` buffers a multi-point connection declaration to the `Harness`; `add_wires(wires)` buffers pre-built `Wire`s; and `field_devices(devices, ...)` registers field-side devices declaratively. All are resolved at build time into their respective `*BuildResult` types and merged into a unified project output.
